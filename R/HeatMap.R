...
#'            session = session) {
#'     shinyId <- shiny::showNotification("Plotting heatmap for log(FC)...", duration = NULL, closeButton = FALSE)
#'     base::on.exit(shiny::removeNotification(shinyId), add = TRUE)
#'     base::tryCatch(
#'       {
#'         startTime <- Sys.time()
#'         base::print(base::paste0(sysTimePID(), " start preparing HM; combinedDFInteractiveHeatMapLog(FC)()"))
...
#'   shinyId <- shiny::showNotification("Plotting condensed heatmap for p-val...", duration = NULL, closeButton = FALSE)
#'   base::on.exit(shiny::removeNotification(shinyId), add = TRUE)
#'   base::tryCatch({
